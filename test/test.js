...
describe("util", () => {
  describe("mat33ToTransform", () => {
    it("should convert valid transforms", () => {
      const A = new Mat33(Vec3(-0.64279, -0.76604, 0), Vec3(0.76604, -0.64279, 0), Vec3(40, 20, 0));
...
describe("parsers", () => {
  describe("parseNumberList", () => {
    ...
    it("should parse x.y.z syntax", () => {
      parseNumberList("-1.7 0-10.6.4-10.5 8.3 0").should.deep.be.equal([
        -1.7, 0, -10.6, 0.4, -10.5, 8.3, 0,
      ]);
    });
  });
  ...
  describe("parseTransform", () => {
    it("should parse translations", () => {
      mat33ToTransform(parseTransform("translate(35 74)")[0]).transform.should.deep.be.equal(
        Transform(Vec2(35, 74), 0),
      );
    });
    it("second argument of translate should be optional", () => {
      mat33ToTransform(parseTransform("translate(22.5)")[0]).transform.should.deep.be.equal(
        Transform(Vec2(22.5, 0), 0),
      );
    });
    it("should parse rotations", () => {
      mat33ToTransform(parseTransform("rotate(-130)")[0]).transform.should.deep.be.almost(
        Transform(Vec2(), (-130 * Math.PI) / 180),
      );
    });
    it("should parse rotations with offset", () => {
      mat33ToTransform(parseTransform("rotate(30, 100, 50)")[0]).transform.should.deep.be.almost(
        Transform(Vec2(38.3974, -43.3012), Math.PI / 6),
      );
    });
    it("should parse matrices", () => {
      mat33ToTransform(
        parseTransform("matrix(-0.64279 -0.76604 0.76604 -0.64279 40 20)")[0],
      ).transform.should.deep.be.almost(Transform(Vec2(40, 20), (-130 * Math.PI) / 180));
    });
  });
  ...
  describe("squashTransforms", () => {
    it("should multiply transforms in right order", () => {
      let a = squashTransforms(
        parseTransforms("translate(100, 100)rotate(-130)", "transform"),
        "transform",
      );
      let b = squashTransforms(
        parseTransforms("matrix(-0.64279 -0.76604 0.76604 -0.64279 100 100)", "transform"),
        "transform",
      );
  ...
  describe("parseCircle", () => {
    it("should apply transformations in right order", () => {
      convertCircle(
        ...
        Transform(Vec2(40, 20), (-130 * Math.PI) / 180),
      ).should.deep.be.almost([Circle(Vec2(42.402117222946615, -42.97640014295341), 50)]);
    });
  });
  describe("parseRect", () => {
    it("should apply correct rotation", () => {
      convertRect(
        ...
        Transform(Vec2(40, 20), (-130 * Math.PI) / 180),
      ).should.deep.be.almost([
        Box(50, 40, Vec2(-23.929702822903984, -16.04891141108515), (260 * Math.PI) / 180),
      ]);
    });
  });
  // TODO
